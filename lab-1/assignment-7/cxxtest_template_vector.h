...
public:
	...
	void test2 () { //Testing the size_t constructor.
		...
		Vector<char> b(100);
		TS_ASSERT_EQUALS(b.size(), 100);
		Vector<testStruct> c((size_t)100);
		c[0].b.push_back(4);
	}
	...
	void test9 () { //Testing the iterators
		Vector<unsigned int> a((size_t)10);
		auto count = 0;
	...
	void test14 () { //Testing the insert member function.
		...
		a.insert(12,999999);
		TS_ASSERT_EQUALS(a[12], 999999);
		TS_ASSERT_EQUALS(a[13], 6);
	...
	void test18 () { //Code copied from the lab directory, I'll deal with turning this into actual testcode later.
		...
		TS_ASSERT_EQUALS(v.size(), 0);      // tom från början
		v.push_back(3.14);          		// lägg till ett element sist 
		TS_ASSERT_EQUALS(v.size(), 1);      // nu ligger ett element i vektorn
		v.insert(0, 2.10);          		// lägg till före element 0, dvs först
		TS_ASSERT_EQUALS(v[0], 2.10);    	// hamnade de rätt?
		TS_ASSERT_EQUALS(v[1], 3.14);       
		TS_ASSERT_EQUALS(v.size(), 2);      // nu ligger två element i vektorn
		v[1] = 2.11;                		// tilldelning av enstaka element;

		const Vector<double> &vc = v;  		// skapa konstant referens
		TS_ASSERT_EQUALS(vc.size(), 2);     // ok: ändrar ej vektorn som är konstant
		TS_ASSERT_EQUALS(vc[0], 2.10);     	// ok: ändrar ej vektorn som är konstant
		TS_ASSERT_EQUALS(vc[1], 2.11);

		v.erase(0);                 		// ta bort första elementet               
		TS_ASSERT_EQUALS(v.size(), 1);      // rätt antal elelment
		v.clear();                  		// töm hela vektorn
		TS_ASSERT_EQUALS(v.size(), 0);      // tom när alla element är borttagna
	}
};